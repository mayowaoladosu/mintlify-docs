---
title: "Pricing"
description: "LayerRail pricing for compute, Game VPS, managed PostgreSQL, Kubernetes, networking, GitHub runners, domains, and AI inference models."
---

Updated on: July 1, 2026

LayerRail pricing is usage-based. A project is billed for the resources it creates, the time those resources run, and metered usage such as AI inference tokens, bandwidth, storage, and domain operations.
...
| Domains | Per domain operation | Registration, renewal, transfer, DNS management, and provider costs where applicable. |
...
Domain pricing depends on the TLD and operation.

| Operation | Pricing behavior |
| --- | --- |
| Registration | One-time yearly registration price for the selected TLD. |
| Renewal | Yearly renewal price for the selected TLD. |
| Transfer | Transfer price where supported by the registry/provider. |
| DNS management | Included for LayerRail-managed DNS zones unless additional infrastructure is attached. |

Core acquisition TLDs can be priced lower for launch and customer acquisition. Some ccTLDs may require a separate registry provider and may have different rules.
